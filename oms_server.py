...
Run this script to start the Order Management System:
    ...
    python oms_server.py
    python oms_server.py --config path/to/config.yaml
...
# On Windows, Python 3.8+ defaults to the Proactor event loop which does not
# support the add_reader API required by zmq. Switch to the Selector loop.
...
async def main(config_path: str = "config.yaml") -> None:
    ...
    # --- Broker ---
    broker = XTSBrokerAdapter(cfg.broker)
    try:
        await broker.login()
